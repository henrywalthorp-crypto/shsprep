...
async function main() {
  ...
  const files = readdirSync(questionsDir).filter(f => f.startsWith('targeted_') && f.endsWith('.json'));

  console.log(`Found ${files.length} targeted question files`);
  ...
  const groups: Record<string, string[]> = {
    'math-exponents': ['math.algebra.exponents', 'math.algebra.exponents_geometry', 'math.scientific_notation', 'math.arithmetic.scientific_notation'],
    'math-inequalities': ['math.algebra.inequalities', 'math.algebra.absolute_value', 'math.absolute_value', 'math.arithmetic.absolute_value'],
